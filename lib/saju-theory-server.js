...
var SJ_UNSUNG_MEANING = {
  ...
  '병':   { spouse:'배우자궁 자리에 변화의 에너지. 관계 전환점이 될 수 있음', career:'직업 변동 주의', child:'자녀 건강 주의', outer:'세상과의 관계에 피로감' },
  '사':   { spouse:'배우자궁 자리의 에너지가 쉬는 위치. 독립적으로 관계를 꾸리는 구조', career:'전환점. 낡은 것을 버려야', child:'자녀와 거리감', outer:'세상과 한발 물러선 관계' },
  '묘':   { spouse:'잠재적 인연. 드러나지 않는 깊은 정', career:'숨어있는 재능 발견', child:'자녀가 조용히 실력 쌓음', outer:'드러나지 않는 저력' },
  '절':   { spouse:'배우자궁 자리에서 인연이 리셋 후 새로 시작하는 패턴', career:'완전히 새로운 출발', child:'자녀와 인연이 끊겼다 이어짐', outer:'세상과 단절 후 재연결' },
...
function SJ_buildUnsungGungwiText(saju, gg) {
  ...
  if (gg && gg.strengthGrade) {
    if (gg.strengthGrade === '신강' || gg.strengthGrade === '극신강') {
      lines.push('  (참고: 전체 에너지는 강한 편. 아래는 각 궁위의 상대적 에너지 흐름)');
    } else if (gg.strengthGrade === '신약' || gg.strengthGrade === '극신약') {
      lines.push('  (참고: 전체 에너지는 약한 편. 아래 궁위별 흐름이 더 민감하게 작용)');
    } else {
      lines.push('  (참고: 전체 에너지는 균형 상태. 아래는 각 궁위의 에너지 흐름)');
    }
  }
...
function SJ_enrichSajuData(saju, gg, dw, gender, mbtiType) {
  ...
  var unsungText = SJ_buildUnsungGungwiText(saju, gg);
...
// ═══════════════════════════════════════════════════
// ── 합충우선순위 이론 (정적 레퍼런스) ──
function SJ_buildHapChungTheory() {
  return '### 합충우선순위 (合沖優先順位)\n' +
'\n' +
'사주 원국에서 같은 지지에 합(合)과 충(沖) 또는 형(刑)이 동시에 걸릴 때, 어느 쪽이 우선하는지 판정하는 원리.\n' +
'\n' +
'■ 탐합망충(貪合忘沖)\n' +
'- 합하는 두 글자가 인접(바로 옆 기둥)하고, 충하는 글자가 멀리 있을 때\n' +
'- 합의 결합력이 충의 파괴력을 흡수하여 충이 무력화됨\n' +
'- 해석: 갈등(충)이 있지만 조화(합)의 힘이 더 강하여 관계가 유지됨\n' +
'\n' +
'■ 합피충파(合被沖破)\n' +
'- 충하는 글자가 합하는 글자 바로 옆(인접)에 있을 때\n' +
'- 충의 파괴력이 합의 결합을 깨뜨림\n' +
'- 해석: 조화를 이루려 했으나 외부 충격으로 관계가 깨지거나 변동이 생김\n' +
'\n' +
'■ 합피형파(合被刑破)\n' +
'- 형(刑), 특히 삼형살(寅巳申 무은지형, 丑戌未 지세지형)이 합에 개입할 때\n' +
'- 형의 마찰력이 합의 결합을 약화시킴\n' +
'- 해석: 내부 갈등이나 구조적 모순으로 조화가 유지되기 어려움\n' +
'\n' +
'■ 합의 결합력 차이\n' +
'- 육합(六合): 1:1 결합으로 결합력 강함. 충/형이 인접해야 깨짐\n' +
'- 삼합(三合): 3글자 결합으로 결합력 강함. 가운데 글자(중심)가 충당하면 깨질 수 있음\n' +
'- 반합(半合): 삼합 중 2글자만 있는 불완전한 합. 결합력이 약하여 충/형에 쉽게 깨짐\n' +
'\n' +
'■ 해석 원칙\n' +
'- 합이 유지되면: 해당 오행의 변화(합화)를 고려, 안정적 에너지로 해석\n' +
'- 합이 깨지면: 원래 글자 각각의 독립적 작용으로 해석, 변동·갈등 암시\n' +
'- 충이 합으로 해소되면: 표면적 갈등이 있으나 내면은 조화로움';
}

// ── 파격 이론 (8대 파격 정적 레퍼런스) ──
function SJ_buildPagyeokTheory() {
  return '### 파격(破格) 이론\n' +
'\n' +
'격국(格局)이 정상적으로 기능하지 못하는 특수 상태. 정격과 해석 방향이 완전히 달라지며, 파격을 해소하는 용신이 핵심.\n' +
'\n' +
'■ 효신탈식(梟神奪食) — 식신격/편인격\n' +
'- 성립: 식신격에서 편인이 식신을 극함 / 편인격에서 식신이 충돌\n' +
'- 해소: 편재가 편인을 제어하면 해소\n' +
'- 해석: 재능과 표현력이 억압받는 구조. 창의성은 있으나 발휘가 막힘. 편재(현실적 행동력)가 돌파구\n' +
'\n' +
'■ 상관견관(傷官見官) — 정관격\n' +
'- 성립: 정관격에서 상관이 정관을 극함\n' +
'- 해소: 인성(정인/편인)이 상관을 설기시키면 해소\n' +
'- 해석: 권위·질서와 자유·반항이 충돌. 조직 내 마찰, 상사와의 갈등. 인성(학습/수양)으로 조화\n' +
'\n' +
'■ 겁재탈재(劫財奪財) — 정재격/편재격\n' +
'- 성립: 재성격에서 겁재가 재물을 빼앗음\n' +
'- 해소: 관성(정관/편관)이 겁재를 제어하면 해소\n' +
'- 해석: 재물 손실, 동료·형제와의 재정 갈등. 관성(규율/체계)으로 안정\n' +
'\n' +
'■ 칠살무제(七殺無制) — 편관격\n' +
'- 성립: 편관격에서 식상(식신+상관)이 부족하여 편관을 제어 못함\n' +
'- 해소: 식상이 충분하면 제살(制殺)로 해소\n' +
'- 해석: 외부 압박·스트레스가 극심. 강한 추진력이 통제 불능. 식상(표현/지혜)으로 제어\n' +
'\n' +
'■ 재다신약(財多身弱)\n' +
'- 성립: 신약한 사주에서 재성이 과다(3 이상)하고 비겁이 부족\n' +
'- 해소: 비겁(비견/겁재)이 충분하면 방신(幇身)으로 해소\n' +
'- 해석: 욕심은 크나 체력·의지가 따라가지 못함. 과로, 건강 악화 주의. 비겁(자기 강화)이 필요\n' +
'\n' +
'■ 관살혼잡(官殺混雜)\n' +
'- 성립: 정관과 편관이 동시에 1개 이상 존재하고 식신으로 제어 안 됨\n' +
'- 해소: 식신이 편관을 제거(제살청관)하거나, 합으로 하나가 제거되면 해소\n' +
'- 해석: 이중 압박, 우유부단, 직업·관계에서 방향성 혼란. 하나를 선택하고 집중해야 함\n' +
'\n' +
'■ 인다용재(印多用財)\n' +
'- 성립: 신강 사주에서 인성이 과다(3 이상)하고 재성이 부족\n' +
'- 해소: 재성이 인성을 제어하면 해소\n' +
'- 해석: 생각만 많고 행동력 부족, 의존적 성향. 재성(현실적 실행력)으로 균형\n' +
'\n' +
'■ 파격 해석 원칙\n' +
'- 파격은 \'나쁜 사주\'가 아니라 \'특수한 과제를 가진 사주\'\n' +
'- 해소 용신이 대운/세운에서 오면 파격이 풀리는 전환점\n' +
'- 파격이 해소되지 않은 시기: 해당 영역에서 시련, 해소된 시기: 큰 도약\n' +
'- 풀이할 때 공포감 주지 말고, 과제와 돌파구를 함께 제시할 것';
}
...
function SJ_buildFullContext(saju, gg, dw, gender, sajuB, ggB) {
  ...
  try { t.push(SJ_buildUnsungGungwiText(saju, gg)); } catch(e) {}
  ...
  // 합충우선순위 + 파격 이론 (정적 레퍼런스 — 모든 사주에 동일하게 주입)
  try { t.push(SJ_buildHapChungTheory()); } catch(e) {}
  try { t.push(SJ_buildPagyeokTheory()); } catch(e) {}
