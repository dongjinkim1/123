...
var STRENGTH_WEIGHTS = {
  ...
  uns: { '제왕':1.5, '건록':1.5, '관대':1.3, '장생':1.2, '목욕':1.0, '양':1.0, '태':0.9, '쇠':0.8, '병':0.7, '사':0.6, '묘':0.6, '절':0.6 },
...
function sipsungToOh(ss, dmEl){
  var i = ohIdx(dmEl);
  if(i<0) return '';
  if(ss==='비견'||ss==='겁재') return OH_LIST[i];
  if(ss==='식신'||ss==='상관') return OH_LIST[(i+1)%5];
  if(ss==='편재'||ss==='정재') return OH_LIST[(i+2)%5];
  if(ss==='편관'||ss==='정관') return OH_LIST[(i+3)%5];
  if(ss==='편인'||ss==='정인') return OH_LIST[(i+4)%5];
  return '';
}
...
function applyDistinctiveness(strengthMap){
  ...
  var total = 0;
  for(var i=0;i<keys.length;i++) total += strengthMap[keys[i]];
...
function calcSinsalStrength(saju){
  ...
  for(var i=0;i<sals.length;i++){
    ...
    result[s.name] = (result[s.name]||0) + base * posMul * STRENGTH_WEIGHTS.category['신살'];
  }
  for(var k in result){
...
function calcRelationStrength(saju){
  ...
  for(var i=0;i<branches.length;i++){
    ...
    for(var j=i+1;j<branches.length;j++){
      ...
      var mul = distMul(i,j) * STRENGTH_WEIGHTS.category['합충'];
...
function extractTopFeatures(saju, gg, opts){
  ...
  Object.keys(sipseongDistinct).forEach(function(name){
    ...
    if(info.score < threshold) return;
    features.push({
      type:'십성', name:name,
      score:info.score, distinct:info.distinct,
      ratio:info.ratio, absolute:info.absolute,
      label:labelStrength(info.ratio)
    });
  });
  Object.keys(sinsal).forEach(function(nm){
    var sc = sinsal[nm];
    if(sc < threshold) return;
    features.push({
      type:'신살', name:nm,
      score:Math.round(sc*100)/100, distinct:Math.round(sc*100)/100,
      ratio:1.0, absolute:Math.round(sc*100)/100,
      label: sc>=1.5?'강':(sc>=1.0?'중':'약')
    });
  });
  Object.keys(relation).forEach(function(nm){
    var sc2 = relation[nm];
    if(sc2 < threshold) return;
    features.push({
      type:'합충', name:nm,
      score:Math.round(sc2*100)/100, distinct:Math.round(sc2*100)/100,
      ratio:1.0, absolute:Math.round(sc2*100)/100,
      label: sc2>=1.0?'강':(sc2>=0.7?'중':'약')
    });
  });

  features.sort(function(a,b){ return b.score - a.score; });
  var topFeatures = features.slice(0, maxN);
  if(topFeatures.length < minN){
    var allSorted = [];
    Object.keys(sipseongDistinct).forEach(function(nm){
      var info = sipseongDistinct[nm];
      allSorted.push({
        type:'십성', name:nm,
        score:info.score, distinct:info.distinct,
        ratio:info.ratio, absolute:info.absolute,
