...
function calcDaewoon(saju, birthY, birthM, birthD, birthH, birthMin, gender){
  var raw=saju.raw;
  // dateToJDN returns the midnight-based JD — add the birth time of day as-is.
  // Solar-term comparison uses the wall clock normalised to KST; true solar time
  // and the 자시 roll are deliberately not applied here.
  var _nb=core.normalizeBirthTime(birthY,birthM,birthD,birthH,birthMin,null);
...
function analyzeDWSEvsWonkuk(saju, dw){
  ...
  // Same list as JIJI_HYUNG; the check below compares both directions.
  ...
  function checkJiRelations(targetJi, targetLabel, targetKr) {
    ...
    wonJis.forEach(function(wj){
      HYUNG_PAIRS.forEach(function(hp){
        // Bidirectional — a one-way check misses e.g. 대운 巳 against 원국 寅.
...
function resolveHapChungPriority(relations){
  ...
  // 삼형살 유형 — 충에 준하는 파괴력 (寅巳申=무은지형, 丑戌未=지세지형)
  // '은혜지형'은 F7에서 폐기된 명칭이라 제거 (丑戌未는 이제 지세지형).
  var STRONG_HYUNG = {'무은지형':1,'지세지형':1};

  function isAdjacent(positionsA, positionsB) {
    for (var i = 0; i < positionsA.length; i++) {
      var ai = posOrder.indexOf(positionsA[i]);
      if (ai < 0) continue;
      for (var j = 0; j < positionsB.length; j++) {
        var bi = posOrder.indexOf(positionsB[j]);
        if (bi >= 0 && Math.abs(ai - bi) <= 1) return true;
      }
    }
    return false;
  }

  // Adjacency must be measured against the side of the 충/형 that is NOT part of
  // the 합. One side is the 합 member itself, so including it makes the distance
  // always 0 and 탐합망충 structurally unreachable (the original defect).
  function outsideOf(hapPositions, positions) {
    return positions.filter(function (p) { return hapPositions.indexOf(p) < 0; });
  }
  function isAdjacentToHap(hapPositions, positions) {
    return isAdjacent(hapPositions, outsideOf(hapPositions, positions));
  }
  ...
  (relations.jijiHap||[]).forEach(function(h){
    haps.push({
      desc: h.desc, strength: 'strong', kind: '육합',
      jis: [h.a.v, h.b.v], positions: [h.a.l, h.b.l]
    });
  });
  (relations.jijiSamhap||[]).forEach(function(h){
    var isSamhap = h.desc && h.desc.indexOf('삼합') >= 0;
    haps.push({
      desc: h.desc,
      strength: isSamhap ? 'strong' : 'medium',  // 반합은 medium
      kind: isSamhap ? '삼합' : '반합',
      jis: (h.members||[]).map(function(m){return m.v;}),
      positions: (h.members||[]).map(function(m){return m.l;})
    });
  });

  var chungs = relations.jijiChung || [];
  var hyungs = relations.jijiHyung || [];
  var resolved = [];
  var usedChungs = {};  // 합에 흡수되거나 합을 깬 충 인덱스
  var usedHyungs = {};

  haps.forEach(function(hap) {
    var broken = false;
    var absorbed = [];  // 강한 합이 흡수한 비인접 충/형

    // ── 충 처리 ──
    chungs.forEach(function(chung, ci) {
      var touches = hap.jis.indexOf(chung.a.v) >= 0 || hap.jis.indexOf(chung.b.v) >= 0;
      if (!touches) return;
      var adj = isAdjacentToHap(hap.positions, [chung.a.l, chung.b.l]);

      if (hap.strength === 'medium') {
        // 반합 — 인접 무관 깨짐
        broken = true;
        resolved.push({type:'합피충파', desc: hap.desc + '(반합)이 ' + chung.desc + '에 의해 깨짐 (반합은 약해 비인접 충에도 무너짐)'});
        usedChungs['c'+ci] = true;
      } else if (adj) {
        // 강한 합 + 인접 충 → 합피충파
        broken = true;
        ...
        // 강한 합 + 비인접 충 → 탐합망충 후보
        absorbed.push({kind:'chung', item:chung, idx:ci});
      }
    });

    // ── 형 처리 ──
    hyungs.forEach(function(hyung, hi) {
      var touches = hap.jis.indexOf(hyung.a.v) >= 0 || hap.jis.indexOf(hyung.b.v) >= 0;
      if (!touches) return;
      var adj = isAdjacentToHap(hap.positions, [hyung.a.l, hyung.b.l]);
      var strong = !!STRONG_HYUNG[hyung.type];

      if (hap.strength === 'medium') {
        // 반합 + 형 — 인접 무관 깨짐
        broken = true;
        resolved.push({type:'합피형파', desc: hap.desc + '(반합)이 ' + hyung.desc + '에 의해 깨짐'});
        usedHyungs['h'+hi] = true;
      } else if (strong && adj) {
        // 강한 합 + 인접 삼형살 → 합피형파
        broken = true;
        resolved.push({type:'합피형파', desc: hap.desc + ' → 인접 ' + hyung.desc + '(삼형살)에 의해 깨짐'});
        usedHyungs['h'+hi] = true;
      } else if (strong && !adj) {
        // 강한 합 + 비인접 삼형살 → 흡수 후보 (탐합망형)
        absorbed.push({kind:'hyung', item:hyung, idx:hi});
      }
      // 약한 형(자형/무례지형 등)이 강한 합과 만나면 형이 약해 합에 영향 못 줌 → 별도 처리 없음
    });

    // 깨지지 않았으면 흡수된 충/형 기록 (탐합망충)
    if (!broken && absorbed.length > 0) {
      absorbed.forEach(function(a) {
        if (a.kind === 'chung') {
          resolved.push({type:'탐합망충', desc: hap.desc + '이 ' + a.item.desc + '을 흡수함 (탐합망충)'});
          usedChungs['c'+a.idx] = true;
        } else {
          resolved.push({type:'탐합망충', desc: hap.desc + '이 ' + a.item.desc + '을 흡수함'});
          usedHyungs['h'+a.idx] = true;
        }
      });
    }

    // 단독 합 "합유지" — 위협이 없는 경우
    //   강한 합: 충/삼형살이 touching하면 위협 (약한 형은 강한 합을 못 깸 → 위협 X)
    //   반합:    모든 충/형이 위협
    var threatChung = chungs.some(function(c){
      return hap.jis.indexOf(c.a.v) >= 0 || hap.jis.indexOf(c.b.v) >= 0;
    });
    var threatHyung = hyungs.some(function(h){
      var touch = hap.jis.indexOf(h.a.v) >= 0 || hap.jis.indexOf(h.b.v) >= 0;
      if (!touch) return false;
      if (hap.strength === 'medium') return true;  // 반합은 모든 형 위협
      return !!STRONG_HYUNG[h.type];                // 강한 합은 삼형살만 위협
    });
    if (!broken && !threatChung && !threatHyung) {
      resolved.push({type:'합유지', desc: hap.desc + ' 유지 (방해 요소 없음)'});
    }
  });

  // 단독 충 — 어떤 합에도 흡수되지 않은 충
  chungs.forEach(function(c, ci) {
    if (!usedChungs['c'+ci]) {
      resolved.push({type:'충작용', desc: c.desc + ' 작용 (합으로 해소되지 않음)'});
    }
  });

  // 단독 형 — 어떤 합에도 흡수되지 않은 형
  hyungs.forEach(function(h, hi) {
    if (!usedHyungs['h'+hi]) {
      resolved.push({type:'형작용', desc: h.desc + ' 작용'});
    }
  });

  return {
    resolved: resolved,
    summary: resolved.map(function(r){return r.desc;}).join(' / ')
  };
}
...
function analyzeGyeokguk(saju){
  ...
  if(!isJonggyeok && !isHwakyeok) {
    ...
    // 일간 강도 (inline 계산 — strengthGrade 변수는 아래에서 계산되므로 여기서는 직접 산출)
    var _pTotal = selfStr + otherStr;
    var _pScore = _pTotal > 0 ? (selfStr / _pTotal * 100) : 50;
    var _isStrong = _pScore >= 60;   // 신강 (극신강 포함)
    var _isWeak = _pScore < 45 && _pScore >= 25;  // 신약 (극신약 제외)

    // 일간 오행 인덱스 (식신/재성 오행 계산용)
    var _ohList = ['목','화','토','금','수'];
    var _dmIdx = _ohList.indexOf(dmEl);
    function _ohOfSs(distance) {  // 일간 기준 N칸 다음 오행 (saeng 순서)
      return _dmIdx >= 0 ? _ohList[(_dmIdx + distance) % 5] : '';
    }
    var _sikOh = _ohOfSs(1);   // 식상 오행
    var _jaeOh = _ohOfSs(2);   // 재성 오행
    ...
    // ── 2. 상관견관 (정관격) — 인성 전체로 해소 조건 확장 ──
    else if(gyeokgukName==='정관격' && (_raw['상관']||0) >= 1 && (_raw['정인']||0) + (_raw['편인']||0) < 1){
      pagyeokInfo = '상관견관(傷官見官) — 상관이 정관을 공격. 능력은 있으나 조직과 충돌';
      yongshin = '인성(제상관) — 인성으로 상관을 누르고 정관을 보호'; yongshinType = '파격조정';
    }
    // ── 3. 겁재탈재 (정재/편재격) — 관성 있으면 해소 ──
    else if((gyeokgukName==='정재격'||gyeokgukName==='편재격') && (_raw['겁재']||0) >= 1 && (_raw['정관']||0) + (_raw['편관']||0) < 1){
      pagyeokInfo = '겁재탈재(劫財奪財) — 재물이 들어와도 빠져나감. 동업·보증 주의';
      yongshin = '관성(제겁재) — 관성으로 겁재를 제어하여 재성 보호'; yongshinType = '파격조정';
    }
    // ── 4. 칠살무제 (편관격) — 식상 전체로 해소 조건 확장, 편인 조건 제거 ──
    else if(gyeokgukName==='편관격' && (_raw['식신']||0) + (_raw['상관']||0) < 0.5){
      pagyeokInfo = '칠살무제(七殺無制) — 편관의 압박을 제어할 식신이 없음. 극심한 스트레스';
      yongshin = '식신(제살) — 식신으로 편관을 제어해야 안정'; yongshinType = '파격조정';
    }
    // ── 5. 효신탈식 (편인격) — 편재 있으면 해소 ──
    else if(gyeokgukName==='편인격' && (_raw['식신']||0) >= 1 && (_raw['편재']||0) < 1){
      pagyeokInfo = '효신탈식(梟神奪食) — 편인이 식신을 극함. 생각만 많고 표현이 막힘';
      yongshin = '편재(제편인) — 편재로 편인을 제어'; yongshinType = '파격조정';
    }
    // ── 6. 관살혼잡 (정관+편관 병존, 식신 부재) ──
    else if((_raw['정관']||0) >= 1 && (_raw['편관']||0) >= 1 && (_raw['식신']||0) < 1){
      pagyeokInfo = '관살혼잡(官殺混雜) — 정관과 편관 병존, 식신으로 제살청관 필요';
      yongshin = '식신(제살청관) — ' + (_sikOh || '식신') + '으로 관성 정리'; yongshinType = '파격조정';
    }
    // ── 7. 재다신약 (신약 + 재성 과다 + 비겁 부족) ──
    else if(_isWeak && (_raw['정재']||0) + (_raw['편재']||0) >= 3 && (_raw['비견']||0) + (_raw['겁재']||0) < 1.5){
      pagyeokInfo = '재다신약(財多身弱) — 재성 과다로 신약, 비겁으로 방신 필요';
      yongshin = '비겁(방신) — ' + (dmEl || '비겁') + '으로 자기 힘 보강'; yongshinType = '파격조정';
    }
    // ── 8. 인다용재 (신강 + 인성 과다 + 재성 부족) ──
    else if(_isStrong && (_raw['정인']||0) + (_raw['편인']||0) >= 3 && (_raw['정재']||0) + (_raw['편재']||0) < 1){
      pagyeokInfo = '인다용재(印多用財) — 인성 과다로 활동력 저하, 재성으로 인성 제어 필요';
      yongshin = '재성(제인성) — ' + (_jaeOh || '재성') + '으로 인성 제어'; yongshinType = '파격조정';
    }
  }
...
function calcRelations(saju){
  ...
  // 지지파(破) — table existed but the check loop was missing, so jijiPa was always empty.
  for(var i=0;i<jis.length;i++)for(var j=i+1;j<jis.length;j++){
...
function calcExtraSinsal(saju){
  ...
  // Canonical table — 묘월 is 申 (branch), not 辛 (stem).
...
function profileAnalysis(saju,gg,rel){
  ...
  // Spouse palace (day branch) sipsung is jiSS[2]; ss[2] is the day STEM and is always 비견.
