...
// ── 강중약 라벨러 (CC-A §14와 동일 규칙: 76/61 컷, 열등 반전) ──
var __PERC_AX = { Ni:1, Ne:1, Si:1, Se:1 }; // 지각기능→S/N(1), 판단기능→T/F(2)
function intBucket(v){ return (v && v>=76)?'강':(v && v>=61)?'중':'약'; }
function mbtiStrengthLabels(type, mbtiIntensities){
  var cf = (TY[type] && TY[type].cf) || '';
  var stack = cf ? cf.split('-') : [];
  var axes = (mbtiIntensities||[]).map(function(v){return intBucket(v);});
  var POSN = ['주','부','3차','열등'];
  var functions = stack.map(function(fn, idx){
    var lab = intBucket((mbtiIntensities||[])[__PERC_AX[fn] ? 1 : 2]);
    if (idx===3) lab = (lab==='강'?'약':lab==='약'?'강':'중');
    return { fn:fn, pos:POSN[idx], label:lab };
  });
  return { type:type, axes:axes, functions:functions };
}
...
module.exports = {
  ...
  mbtiStrengthLabels: mbtiStrengthLabels
};
