...
function buildUserPrompt(params, saju, gg, dw, mt, ti, strArr, mbtiObj) {
  ...
  // MBTS 포인트 주입 (유저 일주×MBTI 조합 1개 entry만)
  try {
    if (_mbtsPoints) {
  ...
  // Pattern injection (server-side: direct require, no typeof check)
  try {
    var userTags = patternEngine.buildUserTags(saju, gg, dw, mt, params.mbtiIntensities);
    // Route through new pattern-matcher (finalScore-based, TRASH-excluded, top-3 per subject).
    // showScores kept on temporarily for verification per spec — can be flipped off later.
