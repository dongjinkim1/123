// ======================================================================
// lib/mbti-theory-server.js — server-side port of public/mbti-theory.js
// Source: public/mbti-theory.js (v1.0). Preserves MT_* logic verbatim.
// Changes vs source:
//   - IIFE removed (CommonJS module)
//   - window.MT_* globals replaced with module.exports at file bottom
//   - Pure data + pure functions, zero runtime dependencies
//   - Body (lines 14~2063) byte-for-byte identical to source
// ======================================================================
'use strict';
...
// 유형 간 관계 조회
var mtGetRelation = function(type1, type2) {
...
// 유형별 커리어 조회
var mtGetCareer = function(code) {
  return MT_CAREER[code] || null;
};

// 유형별 갈등 스타일 조회
var mtGetConflict = function(code) {
...
// 유형 코드 → 풀 프로필 반환
var mtGetType = function(code) {
  return MT_TYPES[code] || null;
};

// 유형 코드 → 스택 위치별 기능 상세 반환
var mtGetStack = function(code) {
  var t = MT_TYPES[code];
  if (!t) return null;
...
// 두 기능의 상호작용 패턴 조회
var mtGetInteraction = function(fn1, fn2) {
...
// 축 + 강도 → 행동 프로필 조회
var mtGetIntensity = function(axis, level) {
...
// ═══════════════════════════════════════════════════
// Module exports (replaces window.* globals)
// ═══════════════════════════════════════════════════
module.exports = {
  // Part 1: 인지기능 / 스택 / 그림자 / 타입 / 상호작용 / 강도 / 발달단계 / 기질
  MT_FUNCTIONS: MT_FUNCTIONS,
  MT_STACK_POSITIONS: MT_STACK_POSITIONS,
  MT_SHADOW_POSITIONS: MT_SHADOW_POSITIONS,
  MT_TYPES: MT_TYPES,
  MT_FUNCTION_INTERACTIONS: MT_FUNCTION_INTERACTIONS,
  MT_INTENSITY_PROFILES: MT_INTENSITY_PROFILES,
  MT_DEVELOPMENT_STAGES: MT_DEVELOPMENT_STAGES,
  MT_TEMPERAMENTS: MT_TEMPERAMENTS,

  // Part 2: 관계 / 커리어 / 갈등 / 축 / 연애 / 스트레스 / 성숙도 / mistype / Big5
  MT_RELATION_TYPES: MT_RELATION_TYPES,
  MT_RELATION_MATRIX: MT_RELATION_MATRIX,
  MT_CAREER: MT_CAREER,
  MT_CONFLICT_STYLES: MT_CONFLICT_STYLES,
  MT_AXES: MT_AXES,
  MT_LOVE: MT_LOVE,
  MT_STRESS_STAGES: MT_STRESS_STAGES,
  MT_MATURITY: MT_MATURITY,
  MT_MISTYPE: MT_MISTYPE,
  MT_BIG_FIVE: MT_BIG_FIVE,
  MT_INTERACTION_STYLES: MT_INTERACTION_STYLES,
  MT_SHADOW_BY_TYPE: MT_SHADOW_BY_TYPE,
  MT_DECISION_PROCESS: MT_DECISION_PROCESS,
  MT_MONEY: MT_MONEY,
  MT_SOCIAL_MEDIA: MT_SOCIAL_MEDIA,
  MT_SELFCARE: MT_SELFCARE,
  MT_FRIENDSHIP: MT_FRIENDSHIP,
  MT_ANGER: MT_ANGER,
  MT_METHODOLOGY: MT_METHODOLOGY,
  MT_PSYCHOMETRIC_STATUS: MT_PSYCHOMETRIC_STATUS,
  MT_REFERENCES: MT_REFERENCES,

  // 헬퍼
  mtGetRelation: mtGetRelation,

  // Builder 함수들 (prompt-builder-usr.js 가 require 해서 사용)
  MT_buildProfileText: MT_buildProfileText,
  MT_buildStackText: MT_buildStackText,
  MT_buildShadowText: MT_buildShadowText,
  MT_buildAxesText: MT_buildAxesText,
  MT_buildStressText: MT_buildStressText,
  MT_buildLoveText: MT_buildLoveText,
  MT_buildConflictText: MT_buildConflictText,
  MT_buildAngerText: MT_buildAngerText,
  MT_buildCareerText: MT_buildCareerText,
  MT_buildDecisionText: MT_buildDecisionText,
  MT_buildMoneyText: MT_buildMoneyText,
  MT_buildSelfcareText: MT_buildSelfcareText,
  MT_buildFriendshipText: MT_buildFriendshipText,
  MT_buildSocialMediaText: MT_buildSocialMediaText,
  MT_buildMaturityText: MT_buildMaturityText,
  MT_buildTemperamentText: MT_buildTemperamentText,
  MT_buildInteractionStyleText: MT_buildInteractionStyleText,
  MT_buildIntensityText: MT_buildIntensityText,
  MT_buildMistypeText: MT_buildMistypeText,
  MT_buildDevelopmentText: MT_buildDevelopmentText,
  MT_buildFullContext: MT_buildFullContext,
  MT_buildRelationText: MT_buildRelationText
};
