...
var sjTheory = require('./saju-theory-server');
...
function buildSajuPrompt(params) {
  ...
  // Inject SJ_ enrichment data (ported from public/saju.js)
  try {
    var sjData = sjTheory.SJ_enrichSajuData(saju, gg, dw, params.gender, mt);
    usr = sjTheory.SJ_injectIntoPrompt(usr, sjData);
  } catch (e) {
    console.warn('[prompt-builder] SJ enrichment failed (falling back to base prompt):', e.message);
  }
