...
// ============================================================
// streamSonnet — 서버용 프롬프트 캡처 전용 (AI 호출 안 함)
// ============================================================
var _capturedPrompts = null;
async function streamSonnet(apiKey, systemPrompt, userMsg, label, callbacks, endpoint) {
  _capturedPrompts = { systemPrompt: systemPrompt, userPrompt: userMsg };
  // server-side: capture prompts only, no actual AI call
  return '{"categories":[]}';
}
function getCapturedPrompts() {
  var p = _capturedPrompts;
  _capturedPrompts = null;
  return p;
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    getAnimalResult: getAnimalResult,
    runSajuAnalysis: runSajuAnalysis,
    getCapturedPrompts: getCapturedPrompts,
    postValidateAI: postValidateAI,
    mkFB: mkFB,
    buildChatPrompt: buildChatPrompt,
    TGAN: TGAN, TGAN_KR: TGAN_KR,
    JIJI: JIJI, JIJI_KR: JIJI_KR,
    TY: TY, DM_AX: DM_AX, MI: MI,
    ANIMALS: ANIMALS, ILJU_DATA: ILJU_DATA,
