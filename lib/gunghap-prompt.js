...
function buildGunghapPrompt(paramsA, paramsB, relType) {
  ...
  try {
    ...
    function buildCharacterProfile(label, userTags) {
      var sections = [];
      for (var i = 0; i < CHAR_SUBS.length; i++) {
        var sub = CHAR_SUBS[i];
        var matched = _patternMatcher.matchPatterns('premium', sub, userTags, 10);
    ...
    userPrompt += buildCharacterProfile(nameA, userTagsA);
    userPrompt += buildCharacterProfile(nameB, userTagsB);
