...
function calculateSaju(year,month,day,hourBranch,hour,minute,dayDate){
  ...
  // ★ 일주는 날짜 기준 — 시각에 영향받지 않는다.
  //   단 정통 자시설(23:00~) 이월이 있으면 dayDate가 익일로 넘어온다.
...
function getSpecialSinsal(yg,yj,mg,mj,dg,dj,hg,hj){
  ...
  // 천덕귀인 정본표(연해자평 가결): 자巳 축庚 인丁 묘申 진壬 사辛 오亥 미甲 신癸 유寅 술丙 해乙
...
// 한국 표준시 UTC+8:30 시행 구간 (KST 환산 시 +30분)
var KOREA_HALFHOUR_FROM=19540321, KOREA_HALFHOUR_TO=19610809;
// 서머타임(+1h) 구간 [시작일,시작분, 종료일,종료분) — 날짜만 있는 구간은 종료일 24:00까지
var KOREA_DST=[
...
// 시각을 deltaMin(분, 소수 허용)만큼 이동 — 날짜 경계를 넘으면 날짜까지 이월
...
// 벽시계(출생 당시 현지 시계) → KST(UTC+9) 기준 시각
// 스프링포워드로 존재하지 않는 시각은 서머타임 적용(관대 해석), 폴백 중복 시각은 서머타임(첫 발생) 우선
...
// 사주 계산용 시각 일괄 정규화
//   kst      — 절기 비교(년주·월주)용 절대시각. 진태양시·자시 이월 적용 안 함(현행 원칙 유지)
//   dayDate  — 일주 계산용 날짜. 진태양시 보정 후 23:00 이상이면 익일
...
function calcExtraSinsal(saju){
  ...
  //    (기존 5주는 을사·무오가 오류. 토 4주를 빼는 8주설도 있으나 12주설 채택 — D3)
