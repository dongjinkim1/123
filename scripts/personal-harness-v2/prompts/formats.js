...
var OUTPUT_SPEC = '## 최종 산출 (JSON 하나만)\n' +
  '{"name":"패턴 제목 1줄(한글, 메커니즘 핵심)","mechanism":"왜 이 조건 조합에서 이 패턴이 생기는가 — 주장 본문","scene":"일상 예시 장면 1개(장식 — 주장 아님)","falsify":"이 패턴이 성립하지 않는 반대 조건 1줄(예: 신약이면 아님)","tags":["원 주문 조건과 같은 개수 유지"]}\n' +
  '주의: tags는 원 주문 조건의 개수를 그대로 유지하라. 태그 추가(패딩) 절대 금지 — 카드가 실제로 보유하지 않은 태그만 카드 보유 태그로 교체 가능하다. 개수를 늘리면 실패작.\n' +
