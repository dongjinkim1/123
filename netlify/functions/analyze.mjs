export default async function handler(req) {
  ...
  try {
    ...
    if (OPENAI_KEY) {
      try {
        console.log('[MBTS] analyze: GPT 호출 (gpt-5.2)');
        ...
        if (gptRes.ok) {
          console.log('[MBTS] analyze: GPT 연결 성공, 스트리밍 시작');
          ...
          console.log('[MBTS] analyze: GPT 실패 (' + gptRes.status + '):', err.slice(0, 200));
        }
      } catch(e) {
    ...
    if (ANTHROPIC_KEY) {
      try {
        ...
        if (claudeRes.ok) {
          console.log('[MBTS] analyze: Claude 연결 성공, 스트리밍 시작');
          ...
          console.log('[MBTS] analyze: Claude 실패:', err.slice(0, 200));
        }
      } catch(e) {
